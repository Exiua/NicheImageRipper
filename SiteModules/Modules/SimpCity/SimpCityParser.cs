...
using NicheImageRipper.SiteModules.Modules.Bunkr;
using NicheImageRipper.SiteModules.Modules.CyberDrop;
using NicheImageRipper.SiteModules.Modules.DotParty;
using NicheImageRipper.SiteModules.Modules.GoFile;
using NicheImageRipper.SiteModules.Modules.Jpg5;
using NicheImageRipper.SiteModules.Modules.PixelDrain;
using NicheImageRipper.SiteModules.Modules.Saint2;
...
public class SimpCityParser : HtmlParser, IHtmlParser
{
    ...
    protected override async Task<RipInfo> Parse(CancellationToken cancellationToken = default)
    {
        ...
        foreach (var (resolveList, i)in resolveLists)
        {
            ...
            foreach (var link in resolveList)
            {
                ...
                try
                {
                    info = await parser.Parse(link, cancellationToken: cancellationToken);
                ...
                catch (WebDriverTimeoutException)
                {
                    ...
                    info = await ReTryParse(link, parser, cancellationToken: cancellationToken);
                }
    ...
    {
        const string cookieName = "dontlikebots_user";
