...
class RipperGui:
    ...
    def __init__(self):
        ...
        self.version: str = 'v1.11.0'

    def app_gui(self):
        """Run the GUI for the Image Ripper"""
        sg.theme(self.theme_color)  # Add a touch of color
        # Tab layouts.
        logger_layout = [[sg.Multiline(size=(90, 20), key='-OUTPUT-', echo_stdout_stderr=True, disabled=True,
                                       write_only=True, reroute_stderr=True, reroute_stdout=True, autoscroll=True)]]
        ...
        history_layout = [[sg.Table(size=(90, 20), values=self.table_data, headings=headings, max_col_width=25,
                                    auto_size_columns=True, display_row_numbers=False, justification='right',
                                    num_rows=9, key='-TABLE-', row_height=35)]]
        settings_layout = [
            [sg.Text('Save Location: '), sg.Text(text=str(self.save_folder), size=(65, 1), key='-FOLDER-')],
            [sg.Text('Select Save Folder:'),
             sg.Input(default_text=self.save_folder, key='-SAVEFOLDER-', visible=False, enable_events=True),
             sg.FolderBrowse(initial_folder=self.save_folder, change_submits=True)],
            [sg.Text('Load Unfinished Urls:'), sg.Input(key='-LOADFILE-', visible=False, enable_events=True),
             sg.FileBrowse(initial_folder='./', file_types=(('JSON Files', '*.json'),), change_submits=True)],
            [sg.Text('Check for updates: '), sg.Button('Check', enable_events=True),
             sg.Text(key='-UPDATE-', size=(50, 1))],
            [sg.Text('Change Theme:'),
             sg.Drop(sg.theme_list(), default_value=self.theme_color, key='-THEME-', enable_events=True)],
            [sg.Text('Filename Scheme:'),
             sg.Drop(("Original", "Hash", "Chronological"), default_value=self.filename_scheme.name.title(),
                     key='-SAVESCHEME-', enable_events=True)],
            [sg.Check('Ask to re-rip url', key='-RERIP-', default=self.rerip_ask, enable_events=True),
             sg.Check('Live update history table', key='-LIVEUPDATE-', default=self.live_history_update,
                      enable_events=True)],
        ...
        # GUI layout
        layout = [[sg.Text('Enter URL: '), sg.InputText(key='-URL-', do_not_clear=False),
                   sg.Button('Rip', bind_return_key=True), sg.Button('Cancel'), sg.Text(key='-STATUS-', size=(20, 1))],
                  [sg.TabGroup([[sg.Tab('Log', logger_layout), sg.Tab('Queue', queue_layout),
                                 sg.Tab('History', history_layout), sg.Tab('Settings', settings_layout)]])]]

        # Create the Window
        window = sg.Window(' '.join(['NicheRipper', self.version]), layout)
        # Separate thread so the queue display can be updated
