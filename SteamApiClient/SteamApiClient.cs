...
public class SteamApiClient
{
    ...
    private readonly ConcurrentDictionary<(uint DepotId, string Host), (TaskCompletionSource<string> Tcs, long ExpiryUnix)> _cdnAuthTokens = new();
    ...
    private void OnLoggedOn(SteamUser.LoggedOnCallback callback)
    {
        ...
        {
            Logger.Error("Unable to logon to Steam: {Result} / {ExtendedResult}", callback.Result, callback.ExtendedResult);
    ...
    private async Task<string?> GetCdnAuthTokenAsync(uint depotId, string host, CancellationToken cancellationToken = default)
    {
        var key = (depotId, host);
        // If we have a valid non-expired token, return it
        if (_cdnAuthTokens.TryGetValue(key, out var existing) && DateTimeOffset.UtcNow.ToUnixTimeSeconds() < existing.ExpiryUnix - 60)
        {
        ...
        try
        {
            ...
            var response = await contentService.GetCDNAuthToken(new CContentServerDirectory_GetCDNAuthToken_Request { depot_id = depotId, host_name = host, app_id = depotId, });
            ...
            Logger.Debug("Got CDN auth token for {Host}, expires {Expiry}", host, DateTimeOffset.FromUnixTimeSeconds(response.Body.expiration_time));
    ...
    public async Task DownloadWorkshopFileAsync(ulong publishedFileId, string outputBaseDirectory, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var detailsResponse = await service.GetDetails(new CPublishedFile_GetDetails_Request { publishedfileids = { publishedFileId }, includeadditionalpreviews = true, includechildren = true, short_description = true, strip_description_bbcode = false, });
            var details = detailsResponse.Body.publishedfiledetails.Single();
            Logger.Information("Workshop item: {Title}, App: {ConsumerAppId}, File: {FileName}, Size: {Size}, HFile: {HFile}", details.title, details.consumer_appid, details.filename, details.file_size, details.hcontent_file);
    ...
    private static async Task DownloadDirectAsync(string url, string outputPath, CancellationToken cancellationToken = default)
    {
    ...
    private async Task<DepotDownloadTarget> ResolveDepotTargetAsync(uint appId, ulong hcontentFile, CancellationToken cancellationToken = default)
    {
        ...
        var response = await directory.CallAsync(HttpMethod.Get, "GetServersForSteamPipe", 1, new Dictionary<string, object?> { ["cell_id"] = _steamClient.CellID ?? 0, ["max_servers"] = 100, });
        var servers = response["servers"].Children.Where(s => s["type"].AsString()is "SteamCache").Where(s => s["https_support"].AsString() == "mandatory").Where(s => (s["vhost"].AsString() ?? s["host"].AsString()!).EndsWith("steamcontent.com")).Select(s => (SteamKit2.CDN.Server)new DnsEndPoint(s["vhost"].AsString() ?? s["host"].AsString()!, 443)).ToList();
        return new DepotDownloadTarget(AppId: appId, ManifestId: hcontentFile, DepotKey: depotKeyResult.DepotKey, ServerPool: new CdnServerPool(servers));
    }
    ...
    private async Task DownloadDepotTargetAsync(DepotDownloadTarget target, string outputPath, CancellationToken cancellationToken = default)
    {
        Logger.Information("Downloading depot target");
        Logger.Debug("App: {AppId}, Manifest: {ManifestId}, Output: {OutputPath}", target.AppId, target.ManifestId, outputPath);
        var contentService = _steamUnifiedMessages.CreateService<ContentServerDirectory>();
        var manifestCodeResponse = await contentService.GetManifestRequestCode(new CContentServerDirectory_GetManifestRequestCode_Request { app_id = target.AppId, depot_id = target.AppId, manifest_id = target.ManifestId, app_branch = "public", });
        ...
        try
        {
            manifest = await _cdnClient.DownloadManifestAsync(depotId: target.AppId, manifestId: target.ManifestId, manifestRequestCode: manifestCodeResponse.Body.manifest_request_code, server: manifestServer, depotKey: target.DepotKey);
        }
        catch
        {
        ...
        try
        {
            ...
            await Parallel.ForEachAsync(chunkQueue, parallelOptions, async (request, ct) =>
            {
                await DownloadChunkWithRetryAsync(request, ct);
            });
        }
    ...
    private async Task DownloadChunkWithRetryAsync(ChunkDownloadRequest request, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            for (var attempt = 0; attempt < maxRetries; attempt++)
            {
                ...
                {
                    throw new InvalidOperationException($"No CDN servers available for chunk at offset {request.Chunk.Offset}.");
                }
                ...
                try
                {
                    ...
                    await _cdnClient.DownloadDepotChunkAsync(request.Target.AppId, request.Chunk, server, destination, request.Target.DepotKey, cdnAuthToken: cdnToken);
                    pool.Return(server);
                    await request.FileLock.WaitAsync(cancellationToken);
                    try
                    {
                        request.FileStream.Seek((long)request.Chunk.Offset, SeekOrigin.Begin);
                        await request.FileStream.WriteAsync(destination.AsMemory(0, (int)request.Chunk.UncompressedLength), cancellationToken);
                    }
                    finally
                    {
                ...
                catch (TaskCanceledException)when (!cancellationToken.IsCancellationRequested)
                {
                    ...
                    Logger.Warning("CDN request timed out on {Host}, server marked broken, retrying in {Delay}s (attempt {Attempt}/{Max})", server.Host, delay.TotalSeconds, attempt + 1, maxRetries);
                ...
                catch (SteamKitWebRequestException ex)when (ex.StatusCode == HttpStatusCode.Forbidden)
                {
                    ...
                    if (!_cdnAuthTokens.TryGetValue(key, out var existingEntry) || existingEntry.Tcs.Task.IsCompleted)
                    {
                        ...
                        await GetCdnAuthTokenAsync(request.Target.AppId, server.Host!, cancellationToken: cancellationToken);
                ...
                catch (SteamKitWebRequestException ex)when (ex.StatusCode is HttpStatusCode.ServiceUnavailable or HttpStatusCode.TooManyRequests or HttpStatusCode.InternalServerError)
                {
                    pool.MarkBroken(server);
                    if (attempt == maxRetries - 1)
                    {
                    ...
                    Logger.Warning("CDN request failed with {Status} on {Host}, server marked broken, retrying in {Delay}s (attempt {Attempt}/{Max})", ex.StatusCode, server.Host, delay.TotalSeconds, attempt + 1, maxRetries);
                    await Task.Delay(delay, cancellationToken);
                    delay *= 2;
                }
                catch
    ...
    public async Task<IReadOnlyList<PublishedFileDetails>> GetUserWorkshopItemsAsync(ulong steamId, uint appId, CancellationToken cancellationToken = default)
    {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            var response = await service.GetUserFiles(new CPublishedFile_GetUserFiles_Request { steamid = steamId, appid = appId, page = page, numperpage = pageSize, sortmethod = "lastupdated", });
            ...
            Logger.Information("Fetched page {Page}, got {Count} items, total so far {Total}", page, files.Count, results.Count);
        ...
        Logger.Information("Found {Total} workshop items for user {SteamId}, app {AppId}", results.Count, steamId, appId);
        return results;
    }
    ...
    public async Task<string> GetPersonaNameAsync(ulong steamId, CancellationToken cancellationToken = default)
    {
        ...
        var response = await service.GetPlayerLinkDetails(new CPlayer_GetPlayerLinkDetails_Request { steamids = { steamId } });
    ...
    public async Task<ulong> ResolveVanityUrlAsync(string vanityUrl, CancellationToken cancellationToken = default)
    {
        ...
        var response = await Http.GetAsync($"https://steamcommunity.com/id/{Uri.EscapeDataString(vanityUrl)}/?xml=1", cancellationToken);
    ...
    private async void OnConnected(SteamClient.ConnectedCallback callback)
    {
        try
        {
            ...
            {
                var authSession = await _steamClient.Authentication.BeginAuthSessionViaCredentialsAsync(new AuthSessionDetails { Username = _username, Password = _password, IsPersistentSession = shouldRememberPassword, GuardData = _previouslyStoredGuardData, Authenticator = new UserConsoleAuthenticator() });
            ...
            _steamUser.LogOn(new SteamUser.LogOnDetails { Username = accountName, AccessToken = refreshToken, ShouldRememberPassword = shouldRememberPassword, });
        }
        catch (Exception e)
        {
