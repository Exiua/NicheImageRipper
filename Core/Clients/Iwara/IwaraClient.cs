...
using NicheImageRipper.Core.Clients.Iwara.Models;

namespace NicheImageRipper.Core.Clients.Iwara;
...
public class IwaraClient
{
    ...
    // Page is 0-based
    public async Task<PagedResponse<Video>?> GetUserVideos(Guid userId, int page, ContentRating rating = ContentRating.All, ContentSort sortBy = ContentSort.Date)
    {
        var success = await CheckToken();
        if (!success)
        {
        ...
        var pagedVideos = await response.Content.ReadFromJsonAsync<PagedResponse<Video>>();
        return pagedVideos;
    }

    public async Task<PagedResponse<Image>?> GetUserImages(Guid userId, int page, ContentRating rating = ContentRating.All, ContentSort sortBy = ContentSort.Date)
    {
        var success = await CheckToken();
        if (!success)
        {
        ...
        var requestUrl = $"https://api.iwara.tv/images?rating={rating.ToUrlString()}&user={userId}&page={page}&sort={sortBy.ToUrlString()}";
        var response = await _httpClient.GetAsync(requestUrl);
        if (!response.IsSuccessStatusCode)
        {
            return null;
        ...
        var pagedImages = await response.Content.ReadFromJsonAsync<PagedResponse<Image>>();
        return pagedImages;
    }
