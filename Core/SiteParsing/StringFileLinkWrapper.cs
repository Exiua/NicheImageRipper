...
public struct StringFileLinkWrapper
{
    ...
    {
        return Url?.Replace(oldValue, newValue) ?? FileLink?.Url.Replace(oldValue, newValue) ?? throw new RipperException("StringImageLinkWrapper is empty.");
    }
    
    public string[] Split(string separator)
    {
        return Url?.Split(separator) ?? FileLink?.Url.Split(separator) ?? throw new RipperException("StringImageLinkWrapper is empty.");
    }
